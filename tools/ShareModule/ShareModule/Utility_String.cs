...
public partial class Utility
{
    ...
    //--------------------------------------------------------
    // 從 UTF8 轉成 ACSII
    public static string FromUTF8ToASCII(string strSource)
    {
        return Encoding.ASCII.GetString(Encoding.GetEncoding("utf-8").GetBytes(strSource));
    }
    // 從 UTF7 轉成 ACSII
    public static string FromUTF7ToASCII(string strSource)
    {
        return Encoding.ASCII.GetString(Encoding.GetEncoding("utf-7").GetBytes(strSource));
    }
    // 從 unicode 轉成 ASCII
    public static string FromUnicodeToASCII(string strSource)
    {
        return Encoding.ASCII.GetString(Encoding.GetEncoding("unicode").GetBytes(strSource));
    }
    // 從 ASCII 轉到 UTF8
    public static string FromASCIIToUTF8(string strSource)
    {
        return Encoding.UTF8.GetString(Encoding.GetEncoding("ascii").GetBytes(strSource));
    }
    ...
    public static bool IsPercentFloat(string strSource)
    {
        strSource = Utility.Replace(strSource, " ", "");
        strSource = Utility.Replace(strSource, "%", "");
        return IsFloat(strSource);
    }
    //------------------------------------------------------------
    // 找尋字串, -1 為找不到
